...
`jawa-dataset` publishes the reference market-observation CSV as an
...
```sh
uv run jawa-dataset publish data/example.csv data/processed \
  --dataset-version 2026-01-02.1 \
  --source-id fixture/example.csv \
  --revision "$(git rev-parse HEAD)" \
  --created-at-utc "$(date -u +%Y-%m-%dT%H:%M:%SZ)"

uv run jawa-dataset verify data/processed/2026-01-02.1
```
